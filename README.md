...
| :--: | :--: |
| 계용운 | 회고1 |
| 서현지 | 이번 프론트엔드 프로젝트를 통해 화면단에서 구현한 내용을 볼 수 있도록 페이지를 구성하고, html, css, vue 등을 통해 실제로 구현해 볼 수 있는 시간이었습니다. 화면 요소 배치나 라이브러리 구성에 있어서 처음 스케치했던 대로 구현하는 것이 생각보다 더 고려해야할 요소가 많다는 것을 느꼈습니다. 이번 프로젝트 기간이 짧아 구상했던 전부를 구현하지는 못해 아쉬웠지만 팀원들과 협력하여 눈에 보이는 결과물을 낸 것 같아 뿌듯합니다! |
| 윤호연 | 피그마부터 Bootstrap과 Vue를 이용한 구현까지 쉽지않은 프론트 프로젝트 였던 것 같습니다. 우선 피그마를 맨땅에 헤딩하는 식으로 스케치하면서 클릭, 호버시 동작하는 것에 대한 부분이 꼬여 이상하게 연결되던 경험도 해보고, 백엔드와는 다르게 코드를 작성하면 바로바로 화면을 통해 확인하고, 수정할 수 있다는 점이 편한 점도 있었지만 저의 얕은 지식으로 생각했던 기능들을 모두 구현하려고 했던 점은 살짝 욕심이었다는 생각도 들었습니다. 프로젝트 기간이 짧아 더 공부하고 찾아보고 해서 만들어내지 못한 채 프로젝트 기간이 끝나 아쉽지만 팀장을 비롯한 모든 팀원들이 함께 노력해서 만들어내어 좋았고, 가능하다면 추가적으로 더 보완해나가고 싶습니다. |
| 이지정 | 프론트를 많이 해보지 않아 미숙했던 점이 많았던 것 같다. 하지만 팀원들이 서로 도와주고 특히 팀장님께서 많이 도와주셔서 짧은 시간내에 프로젝트를 진행할 수 있었던 것 같다. 프론트엔드 프로젝트를 하면서 내가 직접 화면을 디자인 하고 코드를 작성하면 눈에 바로 바로 보였던 점이 재밌었다. |
| 전지연 | 프론트라고하면 html, css, js와 같은 단순한 프론트만 생각했었다. 정보를 받거나 수정하거나 삭제할 수 없고 그저 화면만을 나타낼 수 있는게 프론트라고 생각했었는데, 뷰라는 프레임워크를 사용하면서 프론트에서도 정보를 받거나 수정하거나 삭제할 수 있다는 것을 알게 되었으며 기존에 가지고 있었던 프론트에 대한 고정관념이 좀 깨질 수 있는 시간이었던 것 같다. 사실 프론트를 약간 만만하게 생각했기 때문에 프로젝트로 부딪히면서 미숙하고 부족한 부분으로 인해 많은 어려움을 겪었다. 그러나 팀원들의 도움으로 꼬이거나 미숙하고 어려운 부분을 해결할 수 있었기 때문에 팀원분들께 정말 감사의 인사를 드리고 싶다. |
| 최규범 | 회고6 |
